...
This document is for maintainers of `screen-scaffold`. Library users do not need
to run this repository's tests to use the published artifact.
...
After adding or changing a preview, generate the reference images with
...
Then validate the snapshots before committing with
`./gradlew :screen-scaffold:validateDebugScreenshotTest`.

## Running Tests

Validate the checked-in reference images with
`./gradlew :screen-scaffold:validateDebugScreenshotTest`.

Update reference images after an intentional UI change with
`./gradlew :screen-scaffold:updateDebugScreenshotTest`.
